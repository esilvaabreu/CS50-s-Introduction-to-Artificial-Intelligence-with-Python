        ...
        for movie, state in neighbors_for_person(node.state):
            if not frontier.contains_state(state) and state not in explored_set:
                ...
                if child.state == target:
                    node = child  # for better clarification
                    path = []
...
## Path has to return a list with tuples (movie_id, person_id)
## Return None if there is not a path
## Path = [(none, test1), (m01, aa1), (m02, kb1)]

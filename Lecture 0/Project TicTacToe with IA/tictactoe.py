...
def max_value(board, play, beta=float('inf')):
    if terminal(board):
        return utility(board), play

    v = float('-inf')
    for action in actions(board):
        score, _ = min_value(result(board, action), play=action, alpha=v)
        if score > v:
            v = score
            best_play = action
...
def min_value(board, play, alpha=float('-inf')):
    if terminal(board):
        return utility(board), play

    v = float('inf')
    for action in actions(board):
        score, _ = max_value(result(board, action), play=action, beta=v)
        if score < v:
            v = score
            best_play = action
